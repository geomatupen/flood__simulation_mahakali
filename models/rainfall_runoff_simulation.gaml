...
global {
	...
	// read DEM fil2
	file dem_file <- file("../includes/DEM/western_DEM_Clipped_final1_projected1.tif");
	...
	file watershep_shapefile <- file("../includes/watershed_bdry/modified_mahakali3_projected.shp");

	//create field from dem_file to plot 3D terrain mesh
	grid_file terrain_dem_file <- file("../includes/DEM/western_DEM_Clipped_final1_projected1.tif");
	...
	point bagdwar_location <- point(to_GAMA_CRS({412757.83, 3202244.78}, "EPSG:32644"));

	//	water input start point of Nagmati river
	point dhap_dam_location <- point(to_GAMA_CRS({412757.83, 3202244.78}, "EPSG:32644"));
	...
	init {
		create rainfall_station number: 1 {
			location <- point(to_GAMA_CRS({412757.83, 3202244.78, 227}, "EPSG:32644"));
		}

		create water_level_station number: 1 {
			location <- point(to_GAMA_CRS({412757.83, 3202244.78, 227}, "EPSG:32644")); //2nd_last
		}
...
//			neighbour_cells <- (self neighbors_at 1);
			neighbour_cells <- (self neighbors_at 10);
			neighbour_cells <- neighbour_cells where (each.grid_value < grid_value); //this will speed up the load
			neighbour_cells <- neighbour_cells sort_by (each.grid_value);
			neighbour_cells <- neighbour_cells[0::4];
		}
...
//Altitude of the cell
	...
	action update_color {
		int val_water <- 0;
		val_water <- int((1 - (water_height / (0.05 * 8))) * 255); //it was 10 initially
		if water_height = 0 {
...
experiment Run type: gui until: steps_count = 2 {
